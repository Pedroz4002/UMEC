...
function showPix(data, formData) {
  ...
  setPixStatus("pending", "Aguardando pagamento", "Depois que o Pix for pago, a confirmação aparece aqui automaticamente por até 5 minutos.");
...
compraForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (data.forma_pagamento === "dinheiro") {
      ...
      setMessage(compraMsg, "Pix gerado com sucesso. A confirmação será acompanhada por até 5 minutos.", "success");
    }
  } catch (error) {
